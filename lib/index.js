...
export function replaceParameters(code, parameters, environment) {
    ...
    parameters.forEach((value, key) => {
        if (typeof key === "number") {
            ...
            code = code.replaceAll(new RegExp(`${environment}\\[0x${key.toString(16)}\\]`, 'g'), replacement);
        } else {
            const replacement = debug ? `/* ${environment}[${key}] */ ${value}` : `${value}`;
            code = code.replaceAll(new RegExp(`${environment}\\["${key}"\\]`, 'g'), replacement);
        }
    });
    return code;
}
...
export function simplifyDecoding(code) {
    const visitFunctionDeclaration = function (path) {
        ...
        if (firstStatement.type === "VariableDeclaration"
            && isCodeStringInit(firstStatement.declarations[0])) {
            // Remove the rest element and the length check
            const codeValue = firstStatement.declarations[0].init.value;
        ...
        if (firstStatement.type === "ExpressionStatement" 
            ...
            && firstStatement.expression.callee.type === "Identifier"
            && firstStatement.expression.arguments.length > 0
            && isCodeStringAssignment(firstStatement.expression.arguments[0])) {
            const codeValue = firstStatement.expression.arguments[0].right.value;
            targetNode.body.body = targetNode.body.body.slice(-1);
            const codeValueExpression = {
                type: "StringLiteral",
                value: codeValue
...
export function inlineDecodeHelperCalls(code) {
    function isFirstDecodeHelperCall(statement) {
        if (statement.type !== "FunctionDeclaration") return false;
        if (statement.params.length !== 1
            || statement.params[0].type !== "Identifier"
            || statement.body.body.length !== 3)
            return false;
        const firstStatement = statement.body.body[0];
        const secondStatement = statement.body.body[2];
        if (firstStatement.type !== "VariableDeclaration"
            || firstStatement.declarations.length !== 1
            || firstStatement.declarations[0].init === null
            || firstStatement.declarations[0].init.type !== "CallExpression"
            || firstStatement.declarations[0].init.callee.type !== "Identifier"
            || firstStatement.declarations[0].init.callee.name !== "decodeHelper") return false;
        if (secondStatement.type !== "ReturnStatement"
            || secondStatement.argument.type !== "CallExpression"
            || secondStatement.argument.callee.type !== "Identifier"
            || secondStatement.argument.arguments.length !== 1
            || secondStatement.argument.arguments[0].name !== firstStatement.declarations[0].id.name)
            return false;
        return true;
    }

    const visitBlockStatement = function (path) {
        this.traverse(path);
        const targetNode = path.node
        if (targetNode.body.length === 0) return;

        let blockStatement = targetNode
        for (let i = 0; i < blockStatement.body.length - 1; i++) {
            const statement = blockStatement.body[i];
            if (!isFirstDecodeHelperCall(statement))
                continue;

            // Found a decodeHelper function, inline it
            const functionName = statement.id.name;
            const paramName = statement.params[0].name;
            const firstStatement = statement.body.body[0];
            const secondStatement = statement.body.body[2];
            const wrappedFunction = secondStatement.argument.callee.name;
            const codeString = firstStatement.declarations[0].init.arguments[0];

            var nextFunction = blockStatement.body[i + 1];
            visit(nextFunction, {
                visitCallExpression(path) {
                    const targetNode = path.node
                    if (targetNode.callee.name === functionName && targetNode.arguments.length === 1)
                    {
                        var originalArg = targetNode.arguments[0]
                        targetNode.callee.name = wrappedFunction;
                        targetNode.arguments[0] = {
                            type: "CallExpression",
                            callee: {
                                type: "Identifier",
                                name: "decodeHelper"
                            },
                            arguments: [codeString, originalArg]
                        }
                    }
                    return false;
                    //this.traverse(path);
            ...
            return;
            const callSites = [];
            for (let j = i + 1; j < blockStatement.body.length; j++) {
                const callStatement = blockStatement.body[j];
                if (callStatement.type === "ExpressionStatement"
                    && callStatement.expression.type === "CallExpression"
                    && callStatement.expression.callee.type === "Identifier"
                    && callStatement.expression.callee.name === functionName
                    && callStatement.expression.arguments.length === 1) {
                    callSites.push({
                        index: j,
                        argument: callStatement.expression.arguments[0]
                    });
                }
            }
        }
