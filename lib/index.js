    ...
    code = astToCode(visit(tryParseToAst(code), {
        visitFunctionExpression(path) {
            ...
            if (targetNode.params.length === 1 && targetNode.params[0].type === "RestElement"
                && targetNode.body.body.length > 0 && targetNode.body.body[0].type === "ExpressionStatement"
                && targetNode.body.body[0].expression.type === "AssignmentExpression"
                && targetNode.body.body[0].expression.left.type === "MemberExpression"
                && targetNode.body.body[0].expression.left.object.type === "Identifier"
                // reference to the rest parameter ["length"]
                && targetNode.body.body[0].expression.left.object.name === targetNode.params[0].argument.name
                && targetNode.body.body[0].expression.left.property.value === "length"
                && targetNode.body.body[0].expression.right.type === "NumericLiteral"
                // Should be removed to support all cases.
                && targetNode.body.body[0].expression.right.value === 0
            ) {
                let restName = targetNode.params[0].argument.name;
                //path.params = [];
                targetNode.params = []
                targetNode.body.body = targetNode.body.body.slice(1);
                console.log(restName);
                console.dir(targetNode, { depth: 1 });
                //process.exit(1);
                return false;
            }
            this.traverse(path);
